...
export async function UpdateUser(user){
    ...
    if(user.profile_picture && user.profile_picture[0] && user.profile_picture.length > 0){
    ...
    const user_update_response = await makeRequest(`http://127.0.0.1:8000/users/${user.id}/`,options, true)
