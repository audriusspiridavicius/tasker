...
export default function TaskList({onTaskClick, onDeleteClick}: any) {
    
    const {tasks, isError, isLoading} = useGetTasks()
    const {trigger, isError:err, isMutating, data} = useDeleteTask()
        ...
        {tasks?.map((task:TaskType)=>
            <div key={task.id} className='flex'>
                ...
                <div className='w-1/12 items-center flex justify-center'>
                    ...
                    <DeleteButton className="ml-10 mb-0" onClick={()=>trigger(task.id)}/>
                </div>
            </div>
